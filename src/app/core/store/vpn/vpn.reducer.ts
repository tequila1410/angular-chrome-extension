...
const _vpnReducer = createReducer(
  ...
  on(connectingError, (state, error) => state = {...state, connecting: false, error: error.message}),
  ...
  on(setServersSuccess, (state, data) => state = {...state, serverList: data.serverList, selectedServer: data.serverList[0]})
)
